...
public class ZipExportServiceTest {
  ...
  public void setUp() throws IOException {
    ...
    happyMetadata =
        generateMetadata(SERVICE_YAML_PATH, DEPLOYMENT_YAML_PATH, true, true, true, true);
    happyMetadataBytes = JsonUtils.toPrettyString(happyMetadata).getBytes();
    serviceUrlMetadata =
        generateMetadata(SERVICE_SPEC_URL, DEPLOYMENT_YAML_PATH, true, true, true, true);
  ...
  public void exportObject_happyPathReturnsStream() {
    ...
    expectedOutputStream =
        packZipForExport(happyMetadataBytes, DEPLOYMENT_BYTES, SERVICE_BYTES, PAYLOAD_BYTES);
  ...
  public void exportObject_returnsStreamGivenUrlPaths() {
    ...
    byte[] expectedObject =
        packZipForExport(
                serviceUrlMetadata.toPrettyString().getBytes(),
                DEPLOYMENT_BYTES,
                SERVICE_BYTES,
                PAYLOAD_BYTES)
            .toByteArray();
  ...
  public void exportObject_worksWithNoDeploymentSpec() {
    ...
    byte[] expectedObject =
        packZipForExport(
                noDeploymentMetadata.toPrettyString().getBytes(),
                null,
                SERVICE_BYTES,
                PAYLOAD_BYTES)
            .toByteArray();
  ...
  public void exportObject_worksWithNoServiceSpec() {
    ...
    byte[] expectedObject =
        packZipForExport(
                noServiceSpecMetadata.toPrettyString().getBytes(), DEPLOYMENT_BYTES, null, null)
            .toByteArray();
    byte[] zippedObject = service.exportObject(ARK_ID, KO_PATH, cdoStore).toByteArray();
  ...
  public void exportObject_failsNoDeploymentAnywhere() {
    ...
    byte[] expectedObject =
        packZipForExport(
                noDeploymentMetadata.toPrettyString().getBytes(), null, serviceSpecNoXKgrid, null)
            .toByteArray();
    byte[] zippedObject = service.exportObject(ARK_ID, KO_PATH, cdoStore).toByteArray();
